...
$work_order_controller = new WorkOrderController();

switch ($method) {
  case 'GET':
    if (count($query) < 2) {
      $charges = $work_order_controller->getAll();
      echo $charges;
    } else {
      $id = explode('=', $query[1])[1];
      $charge = $work_order_controller->getOne($id);
      print_r($charge);
    }
    break;
  case 'POST':
    ...
    // insert into db
    $status = $work_order_controller->insert($json);
  ...
  case 'DELETE':
    ...
    $id = explode('=', $query[1])[1];
    // delete from db
    $status = $work_order_controller->destroy($id);
  ...
  case 'PUT':
    ...
    $json = json_decode($reqBody, TRUE);
    // insert into db
    $status = $work_order_controller->update($json, $id);
