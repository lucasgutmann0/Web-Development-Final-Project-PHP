...
switch ($method) {
  case 'GET':
    if (count($query) < 2) {
      $persons = $car_controller->getAll();
      echo $persons;
    } else {
      $id = explode('=', $query[1])[1];
      $person = $car_controller->getOne($id);
      print_r($person);
    }
    break;
  case 'POST':
    ...
    // insert into db
    $status = $car_controller->insert($json);
  ...
  case 'DELETE':
    ...
    $id = explode('=', $query[1])[1];
    // delete from db
    $status = $car_controller->destroy($id);
  ...
  case 'PUT':
    ...
    $json = json_decode($reqBody, TRUE);
    // insert into db
    $status = $car_controller->update($json, $id);
