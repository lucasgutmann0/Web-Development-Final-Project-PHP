...
$charge_controller = new ChargeController();

switch ($method) {
  case 'GET':
    if (count($query) < 2) {
      $charges = $charge_controller->getAll();
      echo $charges;
    } else {
      $id = explode('=', $query[1])[1];
      $charge = $charge_controller->getOne($id);
      print_r($charge);
    }
    break;
  case 'POST':
    ...
    // insert into db
    $status = $charge_controller->insert($json);
  ...
  case 'DELETE':
    ...
    $id = explode('=', $query[1])[1];
    // delete from db
    $status = $charge_controller->destroy($id);
  ...
  case 'PUT':
    ...
    $json = json_decode($reqBody, TRUE);
    // insert into db
    $status = $charge_controller->update($json, $id);
